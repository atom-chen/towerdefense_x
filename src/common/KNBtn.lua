...
function KNBtn:new(path, file, x, y, params, group)
	...
	for i = 1, filsnum	do
		...
		if file[i] ~= "nil" then
			...
			if i == SELECTED then  --默认1，普通，2：选中，3：禁用
				...
				cur:setPosition(-(cur:getContentSize().width - this.item[1]:getContentSize().width ) / 2 + offset[1],
					 -(cur:getContentSize().height - this.item[1]:getContentSize().height ) / 2 + offset[2])
				z = this.params["selectZOrder"] or -1
			else
				z = 0
				--setAnchPos(this.item[i])
				this.item[i]:setPosition(0,0)
			end
	...
	this.layer:setContentSize(CCSize(this.item[1]:getContentSize().width,this.item[1]:getContentSize().height))

	if this.params["front"] then
		if type(this.params["front"]) == "string" then
			this.item["front"] = display.newSprite(this.params["front"])
			this.item["front"]:retain()
			-- setAnchPos(this.item["front"],(this.layer:getContentSize().width) / 2,
			-- 		(this.layer:getContentSize().height) / 2,0.5,0.5)
			this.item["front"]:setPosition((this.layer:getContentSize().width) / 2,
			 		(this.layer:getContentSize().height) / 2)
...
--				local text = display.strokeLabel(v[1], 0, 0, v[2] or 16, ccc3( 0x2c , 0x00 , 0x00 ) , 2 , ccc3( 0x40 , 0x1d , 0x0c ))
				...
				if v[5] then  --若存在则设置对齐方式，否则默认居中
					...
					text:setPosition(tx, (this.layer:getContentSize().height - text:getContentSize().height) / 2 + ty)
				else
					...
					text:setPosition((this.layer:getContentSize().width - text:getContentSize().width) / 2 + tx,
							(this.layer:getContentSize().height - text:getContentSize().height) / 2 + ty)
				end
...
--			end
	...
	--		setAnchPos(bg,60,20)
			state:setPosition(this.params["other"][2] ,this.params["other"][3])
			...
			for k, v in pairs(this.params["other"]) do
				...
				state:setPosition(v[2] ,v[3])
				--setAnchPos(state,v[2] ,v[3])
				this.layer:addChild(state,v[4] or 16)	
			end
		end
--		this.layer:addChild(this.params["other"][1])
--		this.layer:setContentSize(CCSize(this.item[1]:getContentSize().width + bg:getContentSize().width / 2,this.item[1]:getContentSize().height))
	end
	...
	listener:registerScriptHandler(function(touch, event)
		...
		print("began ",x,y)
		-- local nodePoint = this.layer:convertToNodeSpace(location)
		-- x,y=nodePoint.x,nodePoint.y
		print("began --",x,y)
		-- print("----------EVENT_TOUCH_BEGAN:(" .. x .. ", " .. y .. ")")
		if this.state == DISABLE then  --禁 用状态直接返回
			return false
		end
		if this.params["parent"] and 
				not CCRectMake(this.params["parent"]:getX(),    --若有父组件时，点击到组件外部时不响应
						this.params["parent"]:getY(), 
						this.params["parent"]:getWidth(),
		...
		if this:getLayer():getCascadeBoundingBox():containsPoint(cc.p(x,y)) then
			press = true
			moveOn = true
			...
			if not this.params["upSelect"] then  --抬起选中效果时不触发选中状态
				...
				if not this.params["sound_path"] then
					--i7PlaySound("sound/click.mp3", false)
				else
					--i7PlaySound(this.params["sound_path"], false)
				end
			end
			return true
		end
	...
	listener:registerScriptHandler(function(touch, event)
		...
		if this:getLayer():getCascadeBoundingBox():containsPoint(cc.p(x,y)) then
			if press and moveOn then
				--放开后执行回调
				print('&KNBtn=>CCTOUCHENDED',this.params["callback"])
				...
				if this.params["callback"] then
					if not this.params["disableWhenChoose"] then
						result = this.params["callback"]({x = location.x, y = location.y})
					else
						if not this.chosen then
							result = this.params["callback"]({x = location.x, y = location.y})
						end
...
--设置为灰色状态
function KNBtn:setStateView( state )
	print("setStateView",state)
	if state == DISABLE then
		self:showBackgoundView(3)
	elseif state == NORMAL then
		self:showBackgoundView(1)
	end
end
function KNBtn:showBackgoundView( index )
	if self.item[index] then
		for i=1,3 do
			if i~=index then self.item[i]:setVisible(false)
			else self.item[i]:setVisible(true) end
		end
	end
end
	...
	if not done then
		if state == SELECTED then
			if self.params["scale"] then
				...
				cur:setPosition(-(cur:getContentSize().width * scale - cur:getContentSize().width ) / 2,
				  -(cur:getContentSize().height * scale - cur:getContentSize().height ) / 2)
				 if self.item["front"] then
				 	local front = self.item["front"]
...
--	end
	-- print("^(" .. x .. ", " .. y .. ")")
	if not self.params.isIsolated then
	...
	-- print("^KNBtn:getRange|x:", x, "y:", y, "width:", self.layer:getContentSize().width, "height:", self.layer:getContentSize().height)
...
function KNBtn:setOther(sp)
	if self.params["other"] then
		...
		self.params["other"][1]:setPosition(self.params["other"][2],self.params["other"][3])
