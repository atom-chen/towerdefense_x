...
function EditorScene:ctor()
    ...
    self.toolbarView_:setPosition(display.c_left, display.c_bottom + 20)
    ...
    self.returnSetp = require("editor.command.tree").new()
    -- 注册工具栏事件
    self.toolbar_:addEventListener("SELECT_OBJECT", function(event)
        print("EditorScene on Event_select_object")
    ...
    self.toolbar_:addEventListener("PLAY_MAP", function()
        ...
        --self.returnSetp:execute(1)

        
        pushScene("effect")
        -- local layer = require("editor.unities.plistwindow"):create()
        -- pushLayer(self,layer:getLayer())


    end)
    self.toolbar_:addEventListener("Layer",function ()
        local layer =require("editor.component.MapLayer").new()
        self:addChild(layer)

        --self.returnSetp:unexecute()
        -- local property = require("editor.component.PropertyPanel").new()
...
function EditorScene:onTouch(event, x, y)
    ...
    print("EditorScene no runtime touch")
    -- 如果没有运行地图，则将事件传递到工具栏
    x, y = math.round(x), math.round(y)
    print("EditorScene event befor return",event)
    if event == "began" then
