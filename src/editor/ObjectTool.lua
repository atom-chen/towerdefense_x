...
function ObjectTool:onTouchCreateObject(event, x, y)
    ...
    if event == "began" then
        if not self.toolbox_ then
            ...
            local object = self.toolbox_:checkPoint(x, y)--在map生成一个Object & View
            if object then
                --20150907 Sidney add 添加到对象层
                --map的touch事件首先分发到ObjectTool，所以这里作为touch处理的入口也合理
                self.toolbar_:dispatchEvent({name="NEW_OBJECT",object = object})
...
function ObjectTool:onTouchSelectObject(event, x, y)
    if event == "began" then
        for id, object in pairs(self.map_:getAllObjects()) do
            ...
            if (classId == "static" or classId == "entity") and object:checkPointIn(x, y) then
                ...
                print("objecttool onTouchSelectObject return",isDragBegin)
                return isDragBegin
            end
        end
    ...
    elseif event == "moved" then
        print("objecttool onTouchSelectObject moved has behavior",self.currentObject_:hasBehavior("MovableBehavior"))
...
function ObjectTool:onTouch(event, x, y)
    print("ObjectTool , on onTouch",event,x,y,self.selectedButtonName_)
